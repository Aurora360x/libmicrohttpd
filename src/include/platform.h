...
#if defined(_XBOX) && defined(X360_SYSTEMDLL_SUPPORT) && defined(_X360_SYSDLL) && HAVE_XKE_SYSSOCK_H
#include <xkelib.h>
#include <syssock.h>
  // Redefine malloc
  #undef malloc
  #define malloc(size) ExAllocatePoolTypeWithTag(size, 'PTTH', PoolTypeSystem )
  // Redefine free
  #undef free
  #define free(size) ExFreePool(size)
#elif defined(X360_SYSTEMDLL_SUPPORT) && (!defined(_XBOX) || !defined(_X360_SYSDLL) || !HAVE_XKE_SYSSOCK_H)
#error Xbox 360 System DLL support is not supported for a non xbox360 system dll platform or w/o <syssock.h>
#endif
