                    ...
                    void *arg)
{
  ...
  if (stack_size > UINT_MAX)
    ...
    #ifndef _X360_SYSDLL
	HANDLE hThread = (HANDLE)CreateThread(NULL, (DWORD)stack_size, (LPTHREAD_START_ROUTINE)start_routine, arg, CREATE_SUSPENDED, NULL );
    #else 
    HANDLE hThread = INVALID_HANDLE_VALUE;
	DWORD threadId = 0UL;
	ExCreateThread( &hThread, 0, &threadId, (VOID*)XapiThreadStartup, (LPTHREAD_START_ROUTINE)start_routine, arg, 0x2);
    #endif
	if( hThread == INVALID_HANDLE_VALUE ) {
		return 0;
	} 
  
	// Assign thread pointer
	*thread = hThread;
    #ifndef _X360_SYSDLL
