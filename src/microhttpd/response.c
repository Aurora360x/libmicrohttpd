...
#ifdef _XBOX
	MHD_mutex_init_(&response->mutex);
#else
             ...
             size_t max)
{
  ...
  if (_lseeki64 (response->fd,
	...
	#ifdef _XBOX 
	if( fseek( (FILE*)response->fd, (off_t) offset64, SEEK_SET ) != 0)
  ...
  n = fread(buf, 1, max, (FILE*)response->fd);
				 ...
				 void *upgrade_handler_cls)
{
  ...
  response = (struct MHD_Response*)malloc (sizeof (struct MHD_Response));
  if (NULL == response)
    return NULL;
