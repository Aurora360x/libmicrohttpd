...
#elif defined(MHD_WINSOCK_SOCKETS)
#ifndef _XBOX
  if (! SetHandleInformation ((HANDLE)sock, HANDLE_FLAG_INHERIT, 0)) return 0;
#else
  return 1;
#endif
...
#ifdef _XBOX 
  // Set xbox specific socket options
	if( setsockopt(fd, SOL_SOCKET, 0x5802, (char*)&on, sizeof(BOOL) ) != 0 ) {
		DebugBreak();
	}

	if( setsockopt(fd, SOL_SOCKET, 0x5801, (char*)&on, sizeof(BOOL) ) != 0 ) {
		DebugBreak();
	}
#endif
  if (! cloexec_set)
