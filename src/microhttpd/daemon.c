  ...
  if (MHD_NO == may_block)
    {
      timeout.tv_usec = 0;
	  #ifdef _XBOX 
	    timeout.tv_sec = 2;  //changed for slight delay
	  #else 
	    timeout.tv_sec = 0;
	  #endif
      tv = &timeout;
...
#ifdef _XBOX
  MHD_mutex_init_(&daemon->nnc_lock);
#else
  ...
  if ( (MHD_INVALID_SOCKET == daemon->socket_fd) &&
      ...
      if (MHD_INVALID_SOCKET == socket_fd)
	{
      ...
      if (0 == daemon->listening_address_reuse)
        {
#ifdef _XBOX
		if( setsockopt(socket_fd, SOL_SOCKET, 0x5802, (char*)&on, sizeof(BOOL) ) != 0 ) {
			#ifdef HAVE_MESSAGES
				MHD_DLOG (daemon,
						"setsockopt failed: %s\n",
						MHD_socket_last_strerr_ ());
		...
		//if( setsockopt(socket_fd, SOL_SOCKET, 0x5801, (char*)&on, sizeof(BOOL) ) != 0 ) {
		//	#ifdef HAVE_MESSAGES
		//		MHD_DLOG (daemon,
		//				"setsockopt failed: %s\n",
		//				MHD_socket_last_strerr_ ());
...
#ifdef _XBOX
  MHD_mutex_init_( &daemon->per_ip_connection_mutex);
#else
...
#endif
#ifdef _XBOX
  MHD_mutex_init_(&daemon->cleanup_connection_mutex);
#else
...
#ifdef _XBOX
	  MHD_mutex_init_(&d->cleanup_connection_mutex);
#else
...
MHD_stop_daemon (struct MHD_Daemon *daemon)
{
  ...
  if (MHD_ITC_IS_VALID_(daemon->itc))
  ...
  else
    {
      ...
      if ( (MHD_INVALID_SOCKET != fd) && (0 == (daemon->options & MHD_USE_ITC)) )
